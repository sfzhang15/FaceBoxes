...
bool MeetEmitConstraint(const int img_heigh, const int img_width,
  ...
  if (emit_type == EmitConstraint_EmitType_CENTER) {
    ...
    float width = bbox.xmax() * img_width  - bbox.xmin() * img_width + 1;
    float height = bbox.ymax() * img_heigh  - bbox.ymin() * img_heigh + 1;
    // float radius = sqrtf(width*height);
...
//  vector<int> gt_num_matched_anchor(num_gt, 1);
  switch (match_type) {
