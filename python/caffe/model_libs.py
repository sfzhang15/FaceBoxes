...
def ConvBNLayer10(net, from_layer, out_layer, use_bn, use_relu, num_output,
...
def ResBody10(net, from_layer, num_output, expend, eps=0.001):
  ...
  if expend:
    ...
    stride = 2
    net[conv_expand_name] = L.Convolution(net[from_layer], num_output=num_output,
...
def ResNet10Body(net,from_layer):
    ...
    net['data_bn'] = L.BatchNorm(net['data'], in_place=False, **bn_kwargs)
    sb_kwargs = {
        'bias_term': True,
        'param': [dict(lr_mult=1, decay_mult=0), dict(lr_mult=1, decay_mult=0)],
        'filler': dict(type='constant', value=1.0),
    ...
    net['data_scale'] = L.Scale(net['data_bn'], in_place=True, **sb_kwargs)

    ConvBNLayer10(net, 'data_scale', 'conv1', use_bn=True, use_relu=True,
                num_output=64, kernel_size=7, pad=3, stride=2)
    net.conv1_pool = L.Pooling(net.conv1, pool=P.Pooling.MAX, kernel_size=3, stride=2)

    ResBody10(net, 'conv1_pool', 64, False)
    ResBody10(net, 'layer_128_1_relu1', 128, True)
    ResBody10(net, 'layer_256_1_relu1', 256, True)
    ResBody10(net, 'layer_512_1_relu1', 512, True)
