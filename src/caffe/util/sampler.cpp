...
void SampleBBox(const int img_width, const int img_height,
    const Sampler& sampler, NormalizedBBox* sampled_bbox) {
  // Get random scale.
...
void GenerateSamples(const int img_width, const int img_height,
  ...
  for (int i = 0; i < batch_sampler.max_trials(); ++i) {
    ...
    SampleBBox(img_width, img_height, batch_sampler.sampler(), &sampled_bbox);
...
void GenerateBatchSamples(const AnnotatedDatum& anno_datum,
  ...
  for (int i = 0; i < batch_samplers.size(); ++i) {
    if (batch_samplers[i].use_original_image()) {
      ...
      GenerateSamples(img_width, img_height, unit_bbox, object_bboxes, batch_samplers[i],
                      sampled_bboxes);
    }
  }
}
